...
class ContactsViewPopup extends ViewPopup
{
    ...
    function Display() {
	   echo '<script src="custom/modules/Contacts/js/popup_view.js"></script>';
	   if(!empty($_REQUEST["contract_type"])){
		 echo '<script> var contract_type="'.$_REQUEST["contract_type"].'";</script>';
		}
       parent::Display();
	...
	function process() {
		...
		//echo print_r($_SESSION['Contacts2_QUERY']);
