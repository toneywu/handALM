...
if (curl_errno($soap_do)) {
   ...
   $num=count($json_array);
	...
	$customer_id_val = $record['CUSTOMER_ID'];
		...
		$check_customer =BeanFactory::getBean('Accounts')->retrieve_by_string_fields(array('organization_number_c'=>$customer_id_val));
		if (isset($check_customer)) {
			echo "check_customer=true<br/>";
		}else{
			echo "check_customer=false<br/>";
		}
...
/*		$sql = 'SELECT count(1) cnt FROM accounts INNER JOIN accounts_cstm WHERE accounts.id = accounts_cstm.id_c and accounts.deleted=0 AND accounts_cstm.organization_number_c ="'.$customer_id_val.'"';
		...
		/*
	    $count_sql = $query_bean->create_list_count_query($sql);
