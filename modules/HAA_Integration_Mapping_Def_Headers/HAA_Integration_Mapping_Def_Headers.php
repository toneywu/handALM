 ...
 */
require_once('modules/HAA_Integration_Mapping_Def_Headers/HAA_Integration_Mapping_Def_Headers_sugar.php');
class HAA_Integration_Mapping_Def_Headers extends HAA_Integration_Mapping_Def_Headers_sugar {
	...
	function save($check_notify = FALSE){
        ...
        for ($i = 0; $i < $line_count; ++$i) {
        	$lines = new HAA_Integration_Mapping_Def_Lines();
            if ($post_data[$key . 'deleted'][$i] == 1) {
                ...
                $lines->haa_integration_mapping_def_headers_id_c=$this->id;
                $lines->save($check_notify);
                if (!$post_data['line_id'][$i]) {//新建才加关联关系
	            	$table='haa_integra718def_lines_c';
	            	$relate_values = array(
            		'deleted' =>0 ,
	            	'haa_integr33edheaders_ida'=>$this->id,
	            	'haa_integr6553f_lines_idb'=>$lines->id 
	            	);
	            }
