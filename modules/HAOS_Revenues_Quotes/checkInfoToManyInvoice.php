...
//End Add 170108			
			...
			$sql="select contact_id_c,account_id_c,period_name  from haos_revenues_quotes where id in(".$strAll.") group by contact_id_c,account_id_c,period_name";
			$result=$db->query($sql);
			$account=array();
			while ($result_record = $db->fetchByAssoc($result)) {
				//$cord[]=$result_record["result"];
				$account[]=array('contact_id_c'=>$result_record["contact_id_c"],
					'account_id_c'=>$result_record["account_id_c"]);
										//['contact_id_c']=$result_record["contact_id_c"];
				//$account[]['']=$result_record["account_id_c"];
			
			$contractsIdSql = "select id,contact_id_c,account_id_c,period_name  from haos_revenues_quotes where id in(".$strAll.") and contact_id_c='".$result_record["contact_id_c"]."' and 'account_id_c='".$result_record["account_id_c"]."' and period_name='".$result_record["period_name"]."'";

			$contractsIdSqlResult=$db->query($contractsIdSql);
			$idarr=array();
			while ($contractresult_record = $db->fetchByAssoc($contractsIdSqlResult)) {
				$idarr[]=$contractresult_record['id'];
			}
			$str=implode(',', $idarr);
