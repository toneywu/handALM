	...
	*/
	...
	function attr_info($template_arr=array()){
		...
		while ($row = $db->fetchByAssoc($result)) {
			if($asset_id!='' && $row['asset_options']!=''){
				$beanAsset = BeanFactory::getBean('HAT_Assets', $asset_id);
				$asset_info=$beanAsset->$row['asset_options'];
			}
			if($template_arr["module_id"]==''){
				$field_info=array(
			...
			if($template_arr["prodln"]!=''){
				$template_data.=$num%2==0?"<tr>":"";
				if($template_arr["module_action"]=='EditView'){
				$template_data.='<td id="'.$row['column_name'].'_label" data-total-columns="2" scope="col" width="12.5%">'.$row['field_lable'].':';
				$addToValidate='';
				...
				if($template_arr["module_action"]=='EditView'){
					$template_data.=$num%2==0?"":"<div class='row edit-view-row'>";
				}else{
				$template_data.=$num%2==0?"<div class='row detail-view-row'>":"";
				}
				if($template_arr["module_action"]=='EditView'){
				//$template_data.='<div class="col-xs-12 col-sm-4 label">'.$row['field_lable'].':';
				$template_data.='<div class="col-xs-12 col-sm-6 edit-view-row-item"><div class="col-xs-12 col-sm-4 label" style="display:inline-block">'.$row['field_lable'].':';
				$addToValidate='';
				if ($row['required_flag']==1) {
					$template_data.='<span class="required">*</span>';
					if($template_arr["template_type"]=='INV_DETAIL_RESULTS'){
				...
				$template_data.='</div>'.$addToValidate;
			}else if ($template_arr["module_action"]=='DetailView') {
				$template_data.='<div class="col-xs-12 col-sm-6 " style="display:inline-block" ><div class="col-xs-12 col-sm-4 label col-1-label" style="display:inline-block">'.$field_info["field_lable"].':</div>';
			...
			if($template_arr["prodln"]!=''){
				...
				$template_data.=$num%2==0?"</div>":"</div></div>";
			}
			
			$num++;
		}
		if ($template_arr["prodln"]!='' && $num%2!=0) {
	...
	function setLov($field_info){
		...
		if($field_info["column_id"]!=''){
			$beanAttr = BeanFactory::getBean($field_info["relate_module"], $field_info["column_id"]);
			$row_name = $beanAttr->$field_info["module_dsp"];
			$id=$field_info["column_id"];
		}else if ($field_info["asset_info"]!=''){
			$beanAttr = BeanFactory::getBean($field_info["relate_module"], $field_info["asset_info"]);
			$row_name = $beanAttr->$field_info["module_dsp"];
		...
		if($field_info["prodln"]==''){
		$return_html='<div class="col-xs-12 col-sm-8 edit-view-field  yui-ac" type="relate" field="'.$lov_name.'" style="display:inline-block">';
		$return_html="<input name='".$lov_name."' class='sqsEnabled yui-ac-input' tabindex='' id='".$field_info["prefix"].$field_info["column_name"]."_name".$field_info["prodln"]."' size='' value='".$row_name."' title='' autocomplete='off' type='text'";
		$return_html.=$field_info['can_edit_flag']==0?"disabled='disabled'":"";
		$return_html.="><input name='".$lov_id."' id='".$field_info["prefix"].$field_info["column_name"].$field_info["prodln"]."' type='hidden' value='".$id."'><span class='id-ff multiple'>".
		"<button title='选择[Alt+T]' type='button' tabindex='' class='button firstChild' value='选择' name='btn1' onclick='open_popup(\"".$field_info["relate_module"]."\", 600, 400, \"".$field_info["module_filter"]."\", true, false, {\"call_back_function\":\"".$call_back_function."\",\"form_name\":\"EditView\",\"field_to_name_array\":{\"id\":\"".$field_info["prefix"].$field_info["column_name"].$field_info["prodln"]."\",\"name\":\"".$lov_name."\"}}, \"single\", true );'";
		$return_html.=$field_info['can_edit_flag']==0?"disabled='disabled'":"";
		$return_html.="><img src='themes/SuiteP/images/id-ff-select.png?v=6T2wqZkzRRtQXSbbOJRC2A' alt=''></button>".
		"<button type='button' name='btn_clr_".$field_info["column_name"]."' id='btn_clr_".$field_info["column_name"]."' tabindex='' title='清除选择' class='button lastChild' onclick='SUGAR.clearRelateField(this.form, \"".$lov_name."\", \"".$field_info["prefix"].$field_info["column_name"].$field_info["prodln"]."\");".$clear_html.";' value='清除选择'><img src='themes/SuiteP/images/id-ff-clear.png?v=6T2wqZkzRRtQXSbbOJRC2A'></button></span>";
		$return_html.='</div>';
	}else{
		$return_html="<td style='vertical-align: middle;'><input name='".$lov_name."' class='sqsEnabled yui-ac-input' tabindex='0' id='".$field_info["prefix"].$field_info["column_name"]."_name".$field_info["prodln"]."' size='' value='".$row_name."' title='' autocomplete='off' accesskey='7' style='vertical-align: middle;' type='text'";
		$return_html.=$field_info['can_edit_flag']==0?"disabled='disabled'":"";
		$return_html.="><input name='".$lov_id."' id='".$field_info["prefix"].$field_info["column_name"].$field_info["prodln"]."' type='hidden' value='".$id."'>".
		"<button title='选择[Alt+T]' style='vertical-align: middle;' type='button' tabindex='' class='button firstChild' value='选择' name='btn1' onclick='open_popup(\"".$field_info["relate_module"]."\", 600, 400, \"".$field_info["module_filter"]."\", true, false, {\"call_back_function\":\"".$call_back_function."\",\"form_name\":\"EditView\",\"field_to_name_array\":{\"id\":\"".$field_info["prefix"].$field_info["column_name"].$field_info["prodln"]."\",\"name\":\"".$lov_name."\"}}, \"single\", true );'";
		$return_html.=$field_info['can_edit_flag']==0?"disabled='disabled'":"";
		$return_html.="><img src='themes/default/images/id-ff-select.png' alt=''></button>".
		"<button type='button' style='vertical-align: middle;' name='btn_clr_".$field_info["column_name"]."' id='btn_clr_".$field_info["column_name"]."' tabindex='0' title='清除选择' class='button lastChild' onclick='SUGAR.clearRelateField(this.form, \"".$lov_name."\", \"".$field_info["prefix"].$field_info["column_name"].$field_info["prodln"]."\");".$clear_html.";' value='清除选择'><img src='themes/default/images/id-ff-clear.png?v=ehf-FkQ5ENVuqzsrdphKxQ'></button>";
		$return_html.='</td>';
	}
		return $return_html;
	...
	function setLov_detail($field_info){
		...
		if($field_info["column_id"]!=''){
			...
			//var_dump($field_info["relate_module"]."1234321".$field_info["column_id"]);
			//var_dump($beanAttr);
			$row_name = $beanAttr->$field_info["module_dsp"];
			$id=$field_info["column_id"];
		}else if ($field_info["asset_info"]!=''){
			...
			$row_id = $db->fetchByAssoc($result_id);
			$row_name=$row_id["module_dsp"];*/
			//var_dump($field_info["relate_module"]."1234321".$field_info["asset_info"]);
			//var_dump($beanAttr);
			$row_name = $beanAttr->$field_info["module_dsp"];
			$id=$field_info["asset_info"];
		}
		...
		$return_html="";
		$return_html.='<div class="col-xs-12 col-sm-8 detail-view-field "  style="display:inline-block"
		 type="relate" field="'.$field_info["column_name"].'_name" ><a href="?action=ajaxui#ajaxUILoc=index.php%3Fmodule%3D'.$field_info["relate_module"].'%26action%3D'.$template_arr["module_action"].'%26record%3D'.$id.'"><span id="'.$field_info["column_name"].'" class="sugar_field" data-id-value="'.$id.'">'.$row_name.'</span></a></div>';
	...
	function setInput_detail($field_info){
		...
		$return_html='<div class="col-xs-12 col-sm-8 detail-view-field " type="varchar" field="'.$field_info["column_name"].'"><span class="sugar_field" id="'.$field_info["column_name"].'">'.$id.'</span></div>';
		return $return_html;
	}
...
function setList_detail($field_info){
	...
	'.$list_value.'</td>';
	$return_html='<div class="col-xs-12 col-sm-8 detail-view-field " type="enum" field="'.$field_info["column_name"].'"><input class="sugar_field" id="'.$field_info["column_name"].'" value="'.$id.'" type="hidden">'.$list_value.'</div>';
	return $return_html;
}
...
function setCheckbox_detail($field_info){
	...
	$return_html.=$field_info["column_id"]==1?'checked="checked"':'';
	$return_html.='></td>';*/
	$return_html.='<div class="col-xs-12 col-sm-8 detail-view-field " type="bool" field="'.$field_info["column_name"].'"><input class="checkbox" name="'.$field_info["column_name"].'" id="'.$field_info["column_name"].'" value="$fields.'.$field_info["column_name"].'.value" disabled="true" type="checkbox" ';
	$return_html.=$field_info["column_id"]==1?'checked="checked"':'';
	$return_html.='></div>';
	return $return_html;
}
