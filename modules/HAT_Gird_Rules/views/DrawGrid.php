...
<table class="GridTable">
	...
	<?php
		if($tabtype == 1){
			echo '<td class="first col"><div>'. ($j-1) .'</div></td>';
		}
		foreach($av as $k => $v) {
			if ($v['sign'] == 1) {
				echo '<td class="first row"><div>'. $v['name'] .'</div></td>';
			} else {
				if($v == 0){
					echo "<td><div class='empty'></div></td>";
				}else {

					$display_node_name = $v['asname'];
					$regex_mask = '/^[^_]*_[^_]*_[^_]*_/';
					//TODO:为了满足CC的需求，对资产名称进行了截取，目前是写死在当前的代码中，后续需要改为在Gird_rule中进行定义regex
					$display_node_name = preg_replace($regex_mask, '', $display_node_name);

					echo "<td onclick='openGridDetail(\"".$v['asid']."\")' class='color_asset_status_". $v['asset_status']."'><div><span class='asset_name'>";
				    echo $display_node_name . "</span><span class='using_org'>". $v['acname'] . "</span><span class='rack_all'>" . $v['racks'] . "</span>";
				    echo "</div></td>";
				}
			}
		}
