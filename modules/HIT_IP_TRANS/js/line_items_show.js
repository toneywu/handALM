...
function insertTransLineHeader(tableid){
 ...
 $("#LBL_EDITVIEW_PANEL1 tr td:first").remove();
  ...
  j1.innerHTML="<span id='line_backup_asset_title'>网关设备(备)</span>";
...
//包括以下内容：1）显示头，2）定义SQS对象，3）定义界面显示的可见字段，4）界面行编辑器界面
  ...
  x.innerHTML  = "<td colSpan='19'><div class='lineEditor'>"+
    ...
    //vlan通道
      ...
      //网关设备(备)
      "<label>"+"网关设备(备)"+"</label>"+
      "<input class='sqsEnabled' autocomplete='off' type='text' style='width:153px;' name='line_backup_asset[" + prodln + "]' id='line_backup_asset" + prodln + "' value='' title='' >"+
